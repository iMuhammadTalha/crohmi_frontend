            ...
            <div class="animated fadeIn">
            		<div class="container">
            			<div class="form-group">
							...
							<select class="form-control" name="nirMonth" id="nirMonth">
						...
						<div class="form-group">
						<label for="nirYear">Enter Year</label>
						<input type="number" class="form-control" name="nirYear" id="nirYear" min="2018" max="2050" required>
						...
						<div class="form-group">
  							<label for="nirCat">Select Category</label>
							<select class="form-control" name="nirCat" id="nirCat">
            			...
            			<div class="custom-file">
  							<input type="file" class="custom-file-input" name="nirImage" id="nirImage" required onchange="getFileName()">
...
<script type="text/javascript">
    ...
    function postImage(e) {
        ...
        xhr.open('POST', 'http://api.crohmi.ml/api/crohmi/ndvi/map/', true);
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                responseData = xhr.response;
                document.getElementById("uploadedImage").src = xhr.response['nri_image'].replace(/^http:\/\//i, 'https://');
                document.getElementById("ndviImage").src = xhr.response['ndvi_image'].replace(/^http:\/\//i, 'https://');
    ...
    function switchToHealthMap() {
        if (isHealthMap) {
            document.getElementById("uploadedImage").src = responseData['nri_image'].replace(/^http:\/\//i, 'https://');
            document.getElementById("ndviImage").src = responseData['ndvi_image'].replace(/^http:\/\//i, 'https://');
            document.getElementById("switch").innerText = "Switch to Health Map >>";
        } else {
            document.getElementById("uploadedImage").src = responseData['ndvi_image'].replace(/^http:\/\//i, 'https://');
            document.getElementById("ndviImage").src = responseData['health_map'].replace(/^http:\/\//i, 'https://');
